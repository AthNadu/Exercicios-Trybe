...
/*
  ...
  Comportamento:
   ...
   // Faça os ítens de 1 a 3, 5, 7 no arquivo tests/restaurant.spec.js
...
const createMenu = (object) => {
  let menu = {
    fetchMenu: () => object,
      ...
      order: (string) => {
        if (Object.keys(menu.fetchMenu().food).includes(string)) {
          menu.consumption.push(string);
        } else if (Object.keys(menu.fetchMenu().drink).includes(string)) {
          menu.consumption.push(string);
        } else {
      ...
      pay: () => {
        let value = 0;
        menu.consumption.forEach((index) => {
          if (index in menu.fetchMenu().food) {
            value += menu.fetchMenu().food[index];
          }
          if (index in menu.fetchMenu().drink) {
            value += menu.fetchMenu().drink[index];
          }
        });
          let value10 = value * 1.1;
          return value10;
      },
  };
  return menu;
