...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    const resultSeven = createMenu({ food: {coxinha: 5.00, chocolate: 3.00}, drink: {agua: 2.00} });
    resultSeven.order('coxinha');
    resultSeven.order('picanha');
    expect(resultSeven.consumption).toContain('coxinha');
    expect(resultSeven.consumption).not.toContain('picanha');
    expect(resultSeven.order('picanha')).toBe('Item indisponível');
    ...
    resultSeven.order('coxinha');
    resultSeven.order('agua');
    resultSeven.order('chocolate');
    expect(resultSeven.consumption).toEqual(['coxinha', 'coxinha', 'agua', 'chocolate']);
    // 10: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a `consumption`.
    resultSeven.order('coxinha');
    resultSeven.order('coxinha');
    expect(resultSeven.consumption).toEqual(['coxinha', 'coxinha', 'agua', 'chocolate', 'coxinha', 'coxinha']);
    ...
    expect(resultSeven.pay()).toBeCloseTo(((5.00 * 4) + 3.00 + 2.00) * 1.1);
